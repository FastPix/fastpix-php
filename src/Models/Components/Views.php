<?php

/*
* This file is auto-generated.
 * Do not edit this file manually.
 * Your changes will be overwritten during the next generation.
 */

declare(strict_types=1);

namespace FastPix\Sdk\Models\Components;


/** Views - Displays the result of the request. */
class Views
{
    /**
     * Beacon Domain specifies the domain endpoint used by the player or SDK to send analytics or tracking beacons for playback events.
     *
     *
     *
     * @var ?string $beaconDomain
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('beaconDomain')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?string $beaconDomain = null;

    /**
     * User defined metadata. Only accessible once it is enabled in the organization settings.
     *
     *
     *
     * @var ?Custom2 $custom
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('custom')]
    #[\FastPix\Sdk\Serializer\Annotation\Type('\FastPix\Sdk\Models\Components\Custom2|null')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?Custom2 $custom = null;

    /**
     * Exit Before Video Start indicates whether a viewer abandoned the video before it started playing, typically due to long loading times.
     *
     *
     *
     * @var ?bool $exitBeforeVideoStart
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('exitBeforeVideoStart')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?bool $exitBeforeVideoStart = null;

    /**
     * Insert Timestamp refers to the time instance when the view is started.
     *
     *
     *
     * @var ?string $insertTimestamp
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('insertTimestamp')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?string $insertTimestamp = null;

    /**
     * Represents the IP address of the user or device that initiated the playback session.
     *
     *
     *
     * @var ?string $ipAddress
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('ipAddress')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?string $ipAddress = null;

    /**
     * Operating System Version specifies the specific version of the operating system being used by the viewer
     *
     *
     *
     * @var ?string $osVersion
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('osVersion')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?string $osVersion = null;

    /**
     * Player Autoplay On indicates whether the video player automatically initiated playback of the video content.
     *
     *
     *
     * @var ?bool $playerAutoplayOn
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('playerAutoplayOn')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?bool $playerAutoplayOn = null;

    /**
     * Player Preload On indicates whether the player is configured to preload the video content upon page load.
     *
     *
     *
     * @var ?bool $playerPreloadOn
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('playerPreloadOn')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?bool $playerPreloadOn = null;

    /**
     * Player Remote Played specifies if the video is being remotely played to devices such as AirPlay or Chromecast, obtained from the SDK.
     *
     *
     *
     * @var ?bool $playerRemotePlayed
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('playerRemotePlayed')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?bool $playerRemotePlayed = null;

    /**
     * Used Fullscreen denotes whether the viewer utilized the full-screen mode while watching the video.
     *
     *
     *
     * @var ?bool $usedFullScreen
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('usedFullScreen')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?bool $usedFullScreen = null;

    /**
     * Video Startup Failure is a boolean metric indicating whether a viewer encountered an error before the first frame of the video commenced playback.
     *
     *
     *
     * @var ?bool $videoStartupFailed
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('videoStartupFailed')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?bool $videoStartupFailed = null;

    /**
     * View Has Ad is a boolean metric indicating whether an advertisement played or attempted to play during the video view.
     *
     *
     *
     * @var ?bool $viewHasAd
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('viewHasAd')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?bool $viewHasAd = null;

    /**
     * Indicates whether any playback error occurred during the video view. This boolean flag helps identify failed or interrupted playback sessions caused by player, network, or media-related issues.
     *
     *
     *
     * @var ?bool $viewHasError
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('viewHasError')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?bool $viewHasError = null;

    /**
     * View ID is a unique identifier assigned to each individual video viewing session.
     *
     *
     *
     * @var ?string $viewId
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('viewId')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?string $viewId = null;

    /**
     * It is a unique identifier associated with a specific workspace within the FastPix platform.
     *
     *
     *
     * @var ?string $workspaceId
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('workspaceId')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?string $workspaceId = null;

    /**
     * Events specifies the order of events journey of the video playback
     *
     *
     *
     * @var ?array<Event> $events
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('events')]
    #[\FastPix\Sdk\Serializer\Annotation\Type('array<\FastPix\Sdk\Models\Components\Event>|null')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?array $events = null;

    /**
     * The unique identifier assigned to an Autonomous System (AS) on the Internet. The ASN is used to identify and exchange routing information between different networks.
     *
     *
     *
     * @var ?int $asnId
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('asnId')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?int $asnId = null;

    /**
     * The Name associated with the asnId.
     *
     *
     *
     * @var ?string $asnName
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('asnName')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?string $asnName = null;

    /**
     * Average Bitrate represents the average bitrate of the video content watched by the viewer, expressed in bits per second (bps). This metric provides insight into the quality of the video stream.
     *
     *
     *
     * @var ?float $averageBitrate
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('averageBitrate')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?float $averageBitrate = null;

    /**
     * Average Downscaling refers to the average reduction in video resolution or quality during the playback of video content.
     *
     *
     *
     * @var ?float $avgDownscaling
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('avgDownscaling')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?float $avgDownscaling = null;

    /**
     * Average Request Latency average time it takes for a request to be made and processed during video playback
     *
     *
     *
     * @var ?float $avgRequestLatency
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('avgRequestLatency')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?float $avgRequestLatency = null;

    /**
     * Average Request Throughput refers to the average throughput or data transfer rate of HTTP requests made during video playback
     *
     *
     *
     * @var ?float $avgRequestThroughput
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('avgRequestThroughput')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?float $avgRequestThroughput = null;

    /**
     * Average Upscaling refers to the average resolution of the video source is lower than the resolution of the playback device or screen.
     *
     *
     *
     * @var ?float $avgUpscaling
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('avgUpscaling')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?float $avgUpscaling = null;

    /**
     * Browser Engine denotes the rendering engine used by the browser (e.g., Blink, Gecko, WebKit).
     *
     *
     *
     * @var ?string $browserEngine
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('browserEngine')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?string $browserEngine = null;

    /**
     * Browser Name denotes the software application utilized by the viewer to access and watch the video content
     *
     *
     *
     * @var ?string $browserName
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('browserName')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?string $browserName = null;

    /**
     * Browser Version signifies the specific version of the browser software employed by the viewer
     *
     *
     *
     * @var ?string $browserVersion
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('browserVersion')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?string $browserVersion = null;

    /**
     * Buffer Count represents the number of rebuffering events occurring during the video view.
     *
     *
     *
     * @var ?int $bufferCount
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('bufferCount')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?int $bufferCount = null;

    /**
     * Buffer Fill indicates the total time, in milliseconds, that viewers wait for rebuffering per video view.
     *
     *
     *
     * @var ?int $bufferFill
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('bufferFill')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?int $bufferFill = null;

    /**
     * Buffer Frequency measures the rate at which rebuffering events occur, expressed as events per millisecond.
     *
     *
     *
     * @var ?float $bufferFrequency
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('bufferFrequency')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?float $bufferFrequency = null;

    /**
     * Buffer Ratio refers to the percentage of time during video playback where the viewer experiences buffering or rebuffering events.
     *
     *
     *
     * @var ?float $bufferRatio
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('bufferRatio')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?float $bufferRatio = null;

    /**
     * Content Delivery Network (CDN) refers to the network infrastructure responsible for delivering the video content to the viewer.
     *
     *
     *
     * @var ?string $cdn
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('cdn')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?string $cdn = null;

    /**
     * City indicates the geographical location of the viewer accessing the video content.
     *
     *
     *
     * @var ?string $city
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('city')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?string $city = null;

    /**
     * Connection Type signifies the type of network connection utilized by the viewers device
     *
     *
     *
     * @var ?string $connectionType
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('connectionType')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?string $connectionType = null;

    /**
     * Continent represents the continent name of the viewer accessing the video content.
     *
     *
     *
     * @var ?string $continent
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('continent')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?string $continent = null;

    /**
     * Country represents the coded text that represents the country name of viewer accessing the video content.
     *
     *
     *
     * @var ?string $country
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('country')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?string $country = null;

    /**
     * Country Code denotes the two-letter ISO code representing the country of origin for the viewer accessing the video content.
     *
     *
     *
     * @var ?string $countryCode
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('countryCode')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?string $countryCode = null;

    /**
     * Device Manufacturer indicates the brand or manufacturer of the device used by the viewer.
     *
     *
     *
     * @var ?string $deviceManufacturer
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('deviceManufacturer')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?string $deviceManufacturer = null;

    /**
     * Device Model represents the specific model of the device used by the viewer.
     *
     *
     *
     * @var ?string $deviceModel
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('deviceModel')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?string $deviceModel = null;

    /**
     * Device Name refers to the name or label assigned to the device used by the viewer.
     *
     *
     *
     * @var ?string $deviceName
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('deviceName')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?string $deviceName = null;

    /**
     * Device Type denotes the classification of the device used by the viewer
     *
     *
     *
     * @var ?string $deviceType
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('deviceType')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?string $deviceType = null;

    /**
     * DRM Type indicates the type of Digital Rights Management (DRM) utilized during video playback
     *
     *
     *
     * @var ?string $drmType
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('drmType')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?string $drmType = null;

    /**
     * Dropped Frame Count represents the number of frames dropped by the video player during playback.
     *
     *
     *
     * @var ?int $droppedFrameCount
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('droppedFrameCount')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?int $droppedFrameCount = null;

    /**
     * Error Code is an identifier representing a specific type of error that occurred during video playback, potentially leading to playback failure.
     *
     *
     *
     * @var ?string $errorCode
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('errorCode')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?string $errorCode = null;

    /**
     * Specifies the component or stage where the playback error originated, such as streaming, cdn, decoder, or player. This context helps diagnose whether the failure was caused by delivery issues, playback logic, or media decoding problems within the FastPix streaming pipeline.
     *
     *
     *
     * @var ?string $errorContext
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('errorContext')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?string $errorContext = null;

    /**
     * The unique identifier which identifies each type of error that occurs.
     *
     *
     *
     * @var ?int $errorId
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('errorId')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?int $errorId = null;

    /**
     * Error Message is a descriptive message generated by the video player when an error occurs during playback, associated with an error code.
     *
     *
     *
     * @var ?string $errorMessage
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('errorMessage')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?string $errorMessage = null;

    /**
     * Experiment Name is used in A/B testing scenarios to categorize video views into different experiments.
     *
     *
     *
     * @var ?string $experimentName
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('experimentName')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?string $experimentName = null;

    /**
     * Specifies the version of the FastPix API used during data collection or playback reporting. This helps ensure compatibility and traceability between client SDK versions and backend processing.
     *
     *
     *
     * @var ?string $fpApiVersion
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('fpApiVersion')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?string $fpApiVersion = null;

    /**
     * Identifies the type or source of the FastPix player embed used for playback — for example, whether the video was played through a direct player integration, an iframe, or a third-party embedded context. This helps differentiate playback environments and measure performance across embed types.
     *
     *
     *
     * @var ?bool $fpEmbed
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('fpEmbed')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?bool $fpEmbed = null;

    /**
     * Specifies the version of the FastPix embed script or SDK used to initialize the player. This helps track playback behavior and debug issues across different embed versions or deployment environments.
     *
     *
     *
     * @var ?string $fpEmbedVersion
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('fpEmbedVersion')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?string $fpEmbedVersion = null;

    /**
     * FastPix Live Stream ID is the unique identifier associated with a live stream video media within the FastPix Video Platform.
     *
     *
     *
     * @var ?string $fpLiveStreamId
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('fpLiveStreamId')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?string $fpLiveStreamId = null;

    /**
     * FastPix Playback ID refers to the unique identifier associated with the playback instance of a video, particularly used in FastPix Video Platform.
     *
     *
     *
     * @var ?string $fpPlaybackId
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('fpPlaybackId')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?string $fpPlaybackId = null;

    /**
     * FastPix SDK Name identifies the name of the FastPix Player SDK utilized within the player workspace.
     *
     *
     *
     * @var ?string $fpSDK
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('fpSdk')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?string $fpSDK = null;

    /**
     * FastPix SDK Version specifies the version of the FastPix Player SDK integrated into the player.
     *
     *
     *
     * @var ?string $fpSDKVersion
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('fpSdkVersion')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?string $fpSDKVersion = null;

    /**
     * Represents a unique, anonymized identifier assigned to each viewer by the FastPix SDK. This ID helps correlate multiple playback sessions or events to the same viewer across sessions or devices without exposing any personal information.
     *
     *
     *
     * @var ?string $fpViewerId
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('fpViewerId')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?string $fpViewerId = null;

    /**
     * Jump Latency refers to the delay or latency experienced when there is a jump or seek action performed by the viewer while watching a video.
     *
     *
     *
     * @var ?float $jumpLatency
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('jumpLatency')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?float $jumpLatency = null;

    /**
     * Latitude refers to the geographical coordinate representing the north-south position of the viewers location, truncated to one decimal place.
     *
     *
     *
     * @var ?string $latitude
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('latitude')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?string $latitude = null;

    /**
     * Live Stream Latency measures the average time taken from the point of ingest to the point of display for live stream video views.
     *
     *
     *
     * @var ?int $liveStreamLatency
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('liveStreamLatency')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?int $liveStreamLatency = null;

    /**
     * Longitude denotes the geographical coordinate representing the east-west position of the viewers location, truncated to one decimal place.
     *
     *
     *
     * @var ?string $longitude
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('longitude')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?string $longitude = null;

    /**
     * Maximum Downscale Percentage represents the highest percentage of downscaling applied to the video during the view.
     *
     *
     *
     * @var ?float $maxDownscaling
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('maxDownscaling')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?float $maxDownscaling = null;

    /**
     * Max Request Latency refers to the maximum rate of data transfer (throughput) during requests made by the playback.
     *
     *
     *
     * @var ?float $maxRequestLatency
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('maxRequestLatency')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?float $maxRequestLatency = null;

    /**
     * Maximum Upscale Percentage represents the highest percentage of upscaling applied to the video during the view.
     *
     *
     *
     * @var ?float $maxUpscaling
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('maxUpscaling')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?float $maxUpscaling = null;

    /**
     * The media Id value if the video asset is internal to FastPix.
     *
     *
     *
     * @var ?string $mediaId
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('mediaId')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?string $mediaId = null;

    /**
     * Operating System signifies the name of software platform utilized by the viewer.
     *
     *
     *
     * @var ?string $osName
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('osName')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?string $osName = null;

    /**
     * Page Context provides contextual information about the type of page being accessed.
     *
     *
     *
     * @var ?string $pageContext
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('pageContext')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?string $pageContext = null;

    /**
     * Page Load Time measures the time from when the user initiates loading the page to when all resources are loaded on the page.
     *
     *
     *
     * @var ?int $pageLoadTime
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('pageLoadTime')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?int $pageLoadTime = null;

    /**
     * Playback Success Score represents a numerical value indicating the success or quality of the video playback experience.
     *
     *
     *
     * @var ?float $playbackScore
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('playbackScore')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?float $playbackScore = null;

    /**
     * Player Height refers to the vertical dimension, measured in pixels, of the video player as it appears on the webpage.
     *
     *
     *
     * @var string|int|null $playerHeight
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('playerHeight')]
    #[\FastPix\Sdk\Serializer\Annotation\Type('string|int|null')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public string|int|null $playerHeight = null;

    /**
     * Player Initialization Time measures the duration, in milliseconds, from the initialization of the player within the webpage to its readiness to receive further instructions.
     *
     *
     *
     * @var ?int $playerInitializationTime
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('playerInitializationTime')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?int $playerInitializationTime = null;

    /**
     * Player Instance ID is a unique identifier that distinguishes each instance of the Player class created when initializing a video.
     *
     *
     *
     * @var ?string $playerInstanceId
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('playerInstanceId')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?string $playerInstanceId = null;

    /**
     * Player Language indicates the language used for text elements within the video player interface.
     *
     *
     *
     * @var ?string $playerLanguage
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('playerLanguage')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?string $playerLanguage = null;

    /**
     * Player Name serves to differentiate various configurations or types of players used across the website or application.
     *
     *
     *
     * @var ?string $playerName
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('playerName')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?string $playerName = null;

    /**
     * Player Poster refers to the image displayed as a preview before the video playback begins.
     *
     *
     *
     * @var ?string $playerPoster
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('playerPoster')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?string $playerPoster = null;

    /**
     * Player Resolution refers to the resolution of the video player window or viewport where the video content is being displayed.
     *
     *
     *
     * @var ?string $playerResolution
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('playerResolution')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?string $playerResolution = null;

    /**
     * Represents the name of the video player software or framework used for playback (for example, HTML5, HLS.js, Shaka Player).
     *
     *
     *
     * @var ?string $playerSoftwareName
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('playerSoftwareName')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?string $playerSoftwareName = null;

    /**
     * Player Software Version indicates the version number of the player software installed.
     *
     *
     *
     * @var ?string $playerSoftwareVersion
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('playerSoftwareVersion')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?string $playerSoftwareVersion = null;

    /**
     * Specifies the domain or source from which the player was loaded or embedded (for example, stream.fastpix.io or a customer’s custom domain). This helps identify the playback origin and differentiate between various deployment environments.
     *
     *
     *
     * @var ?string $playerSourceDomain
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('playerSourceDomain')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?string $playerSourceDomain = null;

    /**
     * Player Source Height denotes the vertical dimension, measured in pixels, of the source video content being transmitted to the player.
     *
     *
     *
     * @var ?int $playerSourceHeight
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('playerSourceHeight')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?int $playerSourceHeight = null;

    /**
     * Player Source Width represents the width of the source video as perceived by the player, typically measured in pixels.
     *
     *
     *
     * @var ?int $playerSourceWidth
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('playerSourceWidth')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?int $playerSourceWidth = null;

    /**
     * Player Version indicates the version of the player used to render the video content. It is often utilized for performance comparison between different player versions.
     *
     *
     *
     * @var ?string $playerVersion
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('playerVersion')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?string $playerVersion = null;

    /**
     * Represents the total number of times the video player has been initialized or viewed for a specific session or video. This metric helps track playback engagement and identify view patterns across different players or sessions.
     *
     *
     *
     * @var string|int|null $playerViewCount
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('playerViewCount')]
    #[\FastPix\Sdk\Serializer\Annotation\Type('string|int|null')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public string|int|null $playerViewCount = null;

    /**
     * Player Width refers to the width of the player displayed within the webpage, measured in pixels.
     *
     *
     *
     * @var ?int $playerWidth
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('playerWidth')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?int $playerWidth = null;

    /**
     * Represents the unique identifier assigned to a FastPix property, which is associated with a specific workspace or project. It helps link playback and analytics data to the correct property configuration.
     *
     *
     *
     * @var string|int|null $propertyId
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('propertyId')]
    #[\FastPix\Sdk\Serializer\Annotation\Type('string|int|null')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public string|int|null $propertyId = null;

    /**
     * Quality Of Experience Score quantifies the overall viewer experience based on various metrics, providing a decimal score to assess the quality of the viewing experience.
     *
     *
     *
     * @var ?float $qualityOfExperienceScore
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('qualityOfExperienceScore')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?float $qualityOfExperienceScore = null;

    /**
     * Region denotes the geographical region of the viewer accessing the video content.
     *
     *
     *
     * @var ?string $region
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('region')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?string $region = null;

    /**
     * Render Quality Score is a decimal value representing the score indicating the perceived quality of the video.
     *
     *
     *
     * @var ?float $renderQualityScore
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('renderQualityScore')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?float $renderQualityScore = null;

    /**
     * Session ID refers to the unique identifier tracking a viewers session within the FastPix platform.
     *
     *
     *
     * @var ?string $sessionId
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('sessionId')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?string $sessionId = null;

    /**
     * Represents a cryptographic signature used to verify the authenticity and integrity of the playback or API request within the FastPix platform. It ensures that the data has not been tampered with and originates from a trusted source.
     *
     *
     *
     * @var ?string $sign
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('sign')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?string $sign = null;

    /**
     * Stability Score quantifies the smoothness of video playback, typically represented as a decimal value.
     *
     *
     *
     * @var ?float $stabilityScore
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('stabilityScore')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?float $stabilityScore = null;

    /**
     * Startup Score evaluates the startup performance of the player, usually represented as a decimal value
     *
     *
     *
     * @var ?float $startupScore
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('startupScore')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?float $startupScore = null;

    /**
     * Sub Property ID denotes the unique identifier assigned to FastPix properties, previously linked with a specific workspace.
     *
     *
     *
     * @var ?string $subPropertyId
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('subPropertyId')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?string $subPropertyId = null;

    /**
     * Represents the total time (in milliseconds) taken for the video player to start playback from the moment the user initiates the session. This includes loading, buffering, and initialization delays before the first frame is rendered.
     *
     *
     *
     * @var ?int $totalStartupTime
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('totalStartupTime')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?int $totalStartupTime = null;

    /**
     * Updated Timestamp refers to when the record is updated to a particular Video.
     *
     *
     *
     * @var ?string $updatedTimestamp
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('updatedTimestamp')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?string $updatedTimestamp = null;

    /**
     * User Agent represents the user agent string transmitted by the viewers device to identify itself to the server, typically including information about the device and browser.
     *
     *
     *
     * @var ?string $userAgent
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('userAgent')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?string $userAgent = null;

    /**
     * Video Content Type specifies the classification of the video content.
     *
     *
     *
     * @var ?string $videoContentType
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('videoContentType')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?string $videoContentType = null;

    /**
     * Video Duration represents the length of the video, provided in milliseconds, typically supplied to FastPix through custom metadata.
     *
     *
     *
     * @var ?int $videoDuration
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('videoDuration')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?int $videoDuration = null;

    /**
     * Indicates the specific encoding variant or rendition of the video being played, such as resolution, bitrate, or codec type. This helps identify which encoded version of the video was selected for playback.
     *
     *
     *
     * @var ?string $videoEncodingVariant
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('videoEncodingVariant')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?string $videoEncodingVariant = null;

    /**
     * Video ID refers to an internal identifier assigned by the user or system to uniquely identify a particular video.
     *
     *
     *
     * @var ?string $videoId
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('videoId')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?string $videoId = null;

    /**
     * Video Language denotes the primary audio language of the video content, assuming it remains unchanged after playback initiation.
     *
     *
     *
     * @var ?string $videoLanguage
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('videoLanguage')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?string $videoLanguage = null;

    /**
     * Specifies the creator or source responsible for producing the video content.
     *
     *
     *
     * @var ?string $videoProducer
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('videoProducer')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?string $videoProducer = null;

    /**
     * Video Resolution refers to the resolution of the video being played.
     *
     *
     *
     * @var ?string $videoResolution
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('videoResolution')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?string $videoResolution = null;

    /**
     * Video Series denotes the name of a series to which the video content belongs.
     *
     *
     *
     * @var ?string $videoSeries
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('videoSeries')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?string $videoSeries = null;

    /**
     * Video Source Domain identifies the domain from which the video source originates.
     *
     *
     *
     * @var ?string $videoSourceDomain
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('videoSourceDomain')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?string $videoSourceDomain = null;

    /**
     * Video Source Duration represents the duration of the video source content, measured in milliseconds.
     *
     *
     *
     * @var ?int $videoSourceDuration
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('videoSourceDuration')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?int $videoSourceDuration = null;

    /**
     * Video Source Hostname represents the hostname of the video.
     *
     *
     *
     * @var ?string $videoSourceHostname
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('videoSourceHostname')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?string $videoSourceHostname = null;

    /**
     * Video Source Stream Type denotes the type of stream used by the player, although it is currently unused.
     *
     *
     *
     * @var ?string $videoSourceStreamType
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('videoSourceStreamType')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?string $videoSourceStreamType = null;

    /**
     * Video Source Type denotes the format of the video source as determined by the player.
     *
     *
     *
     * @var ?string $videoSourceType
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('videoSourceType')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?string $videoSourceType = null;

    /**
     * Video Source URL refers to the URL of the video source accessed by the player.
     *
     *
     *
     * @var ?string $videoSourceUrl
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('videoSourceUrl')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?string $videoSourceUrl = null;

    /**
     * Video Startup Time measures the duration, in milliseconds, from the initialization of the player within the webpage to its readiness to receive further instructions.
     *
     *
     *
     * @var ?int $videoStartupTime
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('videoStartupTime')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?int $videoStartupTime = null;

    /**
     * Video Title refers to the title of the video content being viewed.
     *
     *
     *
     * @var ?string $videoTitle
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('videoTitle')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?string $videoTitle = null;

    /**
     * Represents the unique identifier for the specific video variant or rendition being played. Each variant corresponds to a particular encoding configuration, such as resolution or bitrate, used for adaptive streaming and performance tracking.
     *
     *
     *
     * @var ?string $videoVariantId
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('videoVariantId')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?string $videoVariantId = null;

    /**
     * Specifies the human-readable name of the video variant or rendition being played (for example, “1080p H.264” or “720p AV1”). This helps identify the playback quality or encoding configuration selected during streaming.
     *
     *
     *
     * @var ?string $videoVariantName
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('videoVariantName')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?string $videoVariantName = null;

    /**
     * View End refers to the date and time, in Coordinated Universal Time (UTC), when the video viewing session concluded.
     *
     *
     *
     * @var ?string $viewEnd
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('viewEnd')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?string $viewEnd = null;

    /**
     * View Max Playhead Position represents the furthest point reached by the playhead during the video view, measured in milliseconds.
     *
     *
     *
     * @var ?int $viewMaxPlayheadPosition
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('viewMaxPlayheadPosition')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?int $viewMaxPlayheadPosition = null;

    /**
     * View Page URL denotes the URL address of the web page where the video content is being accessed.
     *
     *
     *
     * @var ?string $viewPageUrl
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('viewPageUrl')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?string $viewPageUrl = null;

    /**
     * Playing Time denotes the total duration of time the video content was actively playing during the view, excluding time spent buffering, seeking, or joining.
     *
     *
     *
     * @var ?int $viewPlayingTime
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('viewPlayingTime')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?int $viewPlayingTime = null;

    /**
     * View Seeked Count signifies the number of times the viewer attempted to seek to a new location within the video.
     *
     *
     *
     * @var ?int $viewSeekedCount
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('viewSeekedCount')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?int $viewSeekedCount = null;

    /**
     * View Seeked Duration indicates the total duration of time spent waiting for playback to resume after the viewer seeks to a new location. Seek Latency metric in the Dashboard is derived by dividing this value by the view_seek_count.
     *
     *
     *
     * @var ?int $viewSeekedDuration
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('viewSeekedDuration')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?int $viewSeekedDuration = null;

    /**
     * Represents the unique identifier assigned to a single playback session within FastPix. This ID is used to correlate all playback events, errors, and metrics that occur during the same viewing session.
     *
     *
     *
     * @var ?string $viewSessionId
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('viewSessionId')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?string $viewSessionId = null;

    /**
     * View Start refers to the date and time, in Coordinated Universal Time (UTC), when the video viewing session commenced.
     *
     *
     *
     * @var ?string $viewStart
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('viewStart')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?string $viewStart = null;

    /**
     * View Total Content Playback Time represents the cumulative duration of video content watched by the viewer, measured in milliseconds. This metric is internally utilized to calculate upscale and downscale percentages.
     *
     *
     *
     * @var ?int $viewTotalContentPlaybackTime
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('viewTotalContentPlaybackTime')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?int $viewTotalContentPlaybackTime = null;

    /**
     * Viewer ID refers to a customer-defined identifier representing the viewer who is watching the video stream. It must be anonymized and not contain any personally identifiable information.
     *
     *
     *
     * @var ?string $viewerId
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('viewerId')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?string $viewerId = null;

    /**
     * Total Watch Time denotes the total duration of video content watched by the viewer, encompassing startup time, playing time, and potential rebuffering time, measured in milliseconds.
     *
     *
     *
     * @var ?int $watchTime
     */
    #[\FastPix\Sdk\Serializer\Annotation\SerializedName('watchTime')]
    #[\FastPix\Sdk\Serializer\Annotation\SkipWhenNull]
    public ?int $watchTime = null;

    /**
     * @param  ?string  $beaconDomain
     * @param  ?Custom2  $custom
     * @param  ?bool  $exitBeforeVideoStart
     * @param  ?string  $insertTimestamp
     * @param  ?string  $ipAddress
     * @param  ?string  $osVersion
     * @param  ?bool  $playerAutoplayOn
     * @param  ?bool  $playerPreloadOn
     * @param  ?bool  $playerRemotePlayed
     * @param  ?bool  $usedFullScreen
     * @param  ?bool  $videoStartupFailed
     * @param  ?bool  $viewHasAd
     * @param  ?bool  $viewHasError
     * @param  ?string  $viewId
     * @param  ?string  $workspaceId
     * @param  ?array<Event>  $events
     * @param  ?int  $asnId
     * @param  ?string  $asnName
     * @param  ?float  $averageBitrate
     * @param  ?float  $avgDownscaling
     * @param  ?float  $avgRequestLatency
     * @param  ?float  $avgRequestThroughput
     * @param  ?float  $avgUpscaling
     * @param  ?string  $browserEngine
     * @param  ?string  $browserName
     * @param  ?string  $browserVersion
     * @param  ?int  $bufferCount
     * @param  ?int  $bufferFill
     * @param  ?float  $bufferFrequency
     * @param  ?float  $bufferRatio
     * @param  ?string  $cdn
     * @param  ?string  $city
     * @param  ?string  $connectionType
     * @param  ?string  $continent
     * @param  ?string  $country
     * @param  ?string  $countryCode
     * @param  ?string  $deviceManufacturer
     * @param  ?string  $deviceModel
     * @param  ?string  $deviceName
     * @param  ?string  $deviceType
     * @param  ?string  $drmType
     * @param  ?int  $droppedFrameCount
     * @param  ?string  $errorCode
     * @param  ?string  $errorContext
     * @param  ?int  $errorId
     * @param  ?string  $errorMessage
     * @param  ?string  $experimentName
     * @param  ?string  $fpApiVersion
     * @param  ?bool  $fpEmbed
     * @param  ?string  $fpEmbedVersion
     * @param  ?string  $fpLiveStreamId
     * @param  ?string  $fpPlaybackId
     * @param  ?string  $fpSDK
     * @param  ?string  $fpSDKVersion
     * @param  ?string  $fpViewerId
     * @param  ?float  $jumpLatency
     * @param  ?string  $latitude
     * @param  ?int  $liveStreamLatency
     * @param  ?string  $longitude
     * @param  ?float  $maxDownscaling
     * @param  ?float  $maxRequestLatency
     * @param  ?float  $maxUpscaling
     * @param  ?string  $mediaId
     * @param  ?string  $osName
     * @param  ?string  $pageContext
     * @param  ?int  $pageLoadTime
     * @param  ?float  $playbackScore
     * @param  string|int|null  $playerHeight
     * @param  ?int  $playerInitializationTime
     * @param  ?string  $playerInstanceId
     * @param  ?string  $playerLanguage
     * @param  ?string  $playerName
     * @param  ?string  $playerPoster
     * @param  ?string  $playerResolution
     * @param  ?string  $playerSoftwareName
     * @param  ?string  $playerSoftwareVersion
     * @param  ?string  $playerSourceDomain
     * @param  ?int  $playerSourceHeight
     * @param  ?int  $playerSourceWidth
     * @param  ?string  $playerVersion
     * @param  string|int|null  $playerViewCount
     * @param  ?int  $playerWidth
     * @param  string|int|null  $propertyId
     * @param  ?float  $qualityOfExperienceScore
     * @param  ?string  $region
     * @param  ?float  $renderQualityScore
     * @param  ?string  $sessionId
     * @param  ?string  $sign
     * @param  ?float  $stabilityScore
     * @param  ?float  $startupScore
     * @param  ?string  $subPropertyId
     * @param  ?int  $totalStartupTime
     * @param  ?string  $updatedTimestamp
     * @param  ?string  $userAgent
     * @param  ?string  $videoContentType
     * @param  ?int  $videoDuration
     * @param  ?string  $videoEncodingVariant
     * @param  ?string  $videoId
     * @param  ?string  $videoLanguage
     * @param  ?string  $videoProducer
     * @param  ?string  $videoResolution
     * @param  ?string  $videoSeries
     * @param  ?string  $videoSourceDomain
     * @param  ?int  $videoSourceDuration
     * @param  ?string  $videoSourceHostname
     * @param  ?string  $videoSourceStreamType
     * @param  ?string  $videoSourceType
     * @param  ?string  $videoSourceUrl
     * @param  ?int  $videoStartupTime
     * @param  ?string  $videoTitle
     * @param  ?string  $videoVariantId
     * @param  ?string  $videoVariantName
     * @param  ?string  $viewEnd
     * @param  ?int  $viewMaxPlayheadPosition
     * @param  ?string  $viewPageUrl
     * @param  ?int  $viewPlayingTime
     * @param  ?int  $viewSeekedCount
     * @param  ?int  $viewSeekedDuration
     * @param  ?string  $viewSessionId
     * @param  ?string  $viewStart
     * @param  ?int  $viewTotalContentPlaybackTime
     * @param  ?string  $viewerId
     * @param  ?int  $watchTime
     * @phpstan-pure
     */
    public function __construct(?string $beaconDomain = null, ?Custom2 $custom = null, ?bool $exitBeforeVideoStart = null, ?string $insertTimestamp = null, ?string $ipAddress = null, ?string $osVersion = null, ?bool $playerAutoplayOn = null, ?bool $playerPreloadOn = null, ?bool $playerRemotePlayed = null, ?bool $usedFullScreen = null, ?bool $videoStartupFailed = null, ?bool $viewHasAd = null, ?bool $viewHasError = null, ?string $viewId = null, ?string $workspaceId = null, ?array $events = null, ?int $asnId = null, ?string $asnName = null, ?float $averageBitrate = null, ?float $avgDownscaling = null, ?float $avgRequestLatency = null, ?float $avgRequestThroughput = null, ?float $avgUpscaling = null, ?string $browserEngine = null, ?string $browserName = null, ?string $browserVersion = null, ?int $bufferCount = null, ?int $bufferFill = null, ?float $bufferFrequency = null, ?float $bufferRatio = null, ?string $cdn = null, ?string $city = null, ?string $connectionType = null, ?string $continent = null, ?string $country = null, ?string $countryCode = null, ?string $deviceManufacturer = null, ?string $deviceModel = null, ?string $deviceName = null, ?string $deviceType = null, ?string $drmType = null, ?int $droppedFrameCount = null, ?string $errorCode = null, ?string $errorContext = null, ?int $errorId = null, ?string $errorMessage = null, ?string $experimentName = null, ?string $fpApiVersion = null, ?bool $fpEmbed = null, ?string $fpEmbedVersion = null, ?string $fpLiveStreamId = null, ?string $fpPlaybackId = null, ?string $fpSDK = null, ?string $fpSDKVersion = null, ?string $fpViewerId = null, ?float $jumpLatency = null, ?string $latitude = null, ?int $liveStreamLatency = null, ?string $longitude = null, ?float $maxDownscaling = null, ?float $maxRequestLatency = null, ?float $maxUpscaling = null, ?string $mediaId = null, ?string $osName = null, ?string $pageContext = null, ?int $pageLoadTime = null, ?float $playbackScore = null, string|int|null $playerHeight = null, ?int $playerInitializationTime = null, ?string $playerInstanceId = null, ?string $playerLanguage = null, ?string $playerName = null, ?string $playerPoster = null, ?string $playerResolution = null, ?string $playerSoftwareName = null, ?string $playerSoftwareVersion = null, ?string $playerSourceDomain = null, ?int $playerSourceHeight = null, ?int $playerSourceWidth = null, ?string $playerVersion = null, string|int|null $playerViewCount = null, ?int $playerWidth = null, string|int|null $propertyId = null, ?float $qualityOfExperienceScore = null, ?string $region = null, ?float $renderQualityScore = null, ?string $sessionId = null, ?string $sign = null, ?float $stabilityScore = null, ?float $startupScore = null, ?string $subPropertyId = null, ?int $totalStartupTime = null, ?string $updatedTimestamp = null, ?string $userAgent = null, ?string $videoContentType = null, ?int $videoDuration = null, ?string $videoEncodingVariant = null, ?string $videoId = null, ?string $videoLanguage = null, ?string $videoProducer = null, ?string $videoResolution = null, ?string $videoSeries = null, ?string $videoSourceDomain = null, ?int $videoSourceDuration = null, ?string $videoSourceHostname = null, ?string $videoSourceStreamType = null, ?string $videoSourceType = null, ?string $videoSourceUrl = null, ?int $videoStartupTime = null, ?string $videoTitle = null, ?string $videoVariantId = null, ?string $videoVariantName = null, ?string $viewEnd = null, ?int $viewMaxPlayheadPosition = null, ?string $viewPageUrl = null, ?int $viewPlayingTime = null, ?int $viewSeekedCount = null, ?int $viewSeekedDuration = null, ?string $viewSessionId = null, ?string $viewStart = null, ?int $viewTotalContentPlaybackTime = null, ?string $viewerId = null, ?int $watchTime = null)
    {
        $this->beaconDomain = $beaconDomain;
        $this->custom = $custom;
        $this->exitBeforeVideoStart = $exitBeforeVideoStart;
        $this->insertTimestamp = $insertTimestamp;
        $this->ipAddress = $ipAddress;
        $this->osVersion = $osVersion;
        $this->playerAutoplayOn = $playerAutoplayOn;
        $this->playerPreloadOn = $playerPreloadOn;
        $this->playerRemotePlayed = $playerRemotePlayed;
        $this->usedFullScreen = $usedFullScreen;
        $this->videoStartupFailed = $videoStartupFailed;
        $this->viewHasAd = $viewHasAd;
        $this->viewHasError = $viewHasError;
        $this->viewId = $viewId;
        $this->workspaceId = $workspaceId;
        $this->events = $events;
        $this->asnId = $asnId;
        $this->asnName = $asnName;
        $this->averageBitrate = $averageBitrate;
        $this->avgDownscaling = $avgDownscaling;
        $this->avgRequestLatency = $avgRequestLatency;
        $this->avgRequestThroughput = $avgRequestThroughput;
        $this->avgUpscaling = $avgUpscaling;
        $this->browserEngine = $browserEngine;
        $this->browserName = $browserName;
        $this->browserVersion = $browserVersion;
        $this->bufferCount = $bufferCount;
        $this->bufferFill = $bufferFill;
        $this->bufferFrequency = $bufferFrequency;
        $this->bufferRatio = $bufferRatio;
        $this->cdn = $cdn;
        $this->city = $city;
        $this->connectionType = $connectionType;
        $this->continent = $continent;
        $this->country = $country;
        $this->countryCode = $countryCode;
        $this->deviceManufacturer = $deviceManufacturer;
        $this->deviceModel = $deviceModel;
        $this->deviceName = $deviceName;
        $this->deviceType = $deviceType;
        $this->drmType = $drmType;
        $this->droppedFrameCount = $droppedFrameCount;
        $this->errorCode = $errorCode;
        $this->errorContext = $errorContext;
        $this->errorId = $errorId;
        $this->errorMessage = $errorMessage;
        $this->experimentName = $experimentName;
        $this->fpApiVersion = $fpApiVersion;
        $this->fpEmbed = $fpEmbed;
        $this->fpEmbedVersion = $fpEmbedVersion;
        $this->fpLiveStreamId = $fpLiveStreamId;
        $this->fpPlaybackId = $fpPlaybackId;
        $this->fpSDK = $fpSDK;
        $this->fpSDKVersion = $fpSDKVersion;
        $this->fpViewerId = $fpViewerId;
        $this->jumpLatency = $jumpLatency;
        $this->latitude = $latitude;
        $this->liveStreamLatency = $liveStreamLatency;
        $this->longitude = $longitude;
        $this->maxDownscaling = $maxDownscaling;
        $this->maxRequestLatency = $maxRequestLatency;
        $this->maxUpscaling = $maxUpscaling;
        $this->mediaId = $mediaId;
        $this->osName = $osName;
        $this->pageContext = $pageContext;
        $this->pageLoadTime = $pageLoadTime;
        $this->playbackScore = $playbackScore;
        $this->playerHeight = $playerHeight;
        $this->playerInitializationTime = $playerInitializationTime;
        $this->playerInstanceId = $playerInstanceId;
        $this->playerLanguage = $playerLanguage;
        $this->playerName = $playerName;
        $this->playerPoster = $playerPoster;
        $this->playerResolution = $playerResolution;
        $this->playerSoftwareName = $playerSoftwareName;
        $this->playerSoftwareVersion = $playerSoftwareVersion;
        $this->playerSourceDomain = $playerSourceDomain;
        $this->playerSourceHeight = $playerSourceHeight;
        $this->playerSourceWidth = $playerSourceWidth;
        $this->playerVersion = $playerVersion;
        $this->playerViewCount = $playerViewCount;
        $this->playerWidth = $playerWidth;
        $this->propertyId = $propertyId;
        $this->qualityOfExperienceScore = $qualityOfExperienceScore;
        $this->region = $region;
        $this->renderQualityScore = $renderQualityScore;
        $this->sessionId = $sessionId;
        $this->sign = $sign;
        $this->stabilityScore = $stabilityScore;
        $this->startupScore = $startupScore;
        $this->subPropertyId = $subPropertyId;
        $this->totalStartupTime = $totalStartupTime;
        $this->updatedTimestamp = $updatedTimestamp;
        $this->userAgent = $userAgent;
        $this->videoContentType = $videoContentType;
        $this->videoDuration = $videoDuration;
        $this->videoEncodingVariant = $videoEncodingVariant;
        $this->videoId = $videoId;
        $this->videoLanguage = $videoLanguage;
        $this->videoProducer = $videoProducer;
        $this->videoResolution = $videoResolution;
        $this->videoSeries = $videoSeries;
        $this->videoSourceDomain = $videoSourceDomain;
        $this->videoSourceDuration = $videoSourceDuration;
        $this->videoSourceHostname = $videoSourceHostname;
        $this->videoSourceStreamType = $videoSourceStreamType;
        $this->videoSourceType = $videoSourceType;
        $this->videoSourceUrl = $videoSourceUrl;
        $this->videoStartupTime = $videoStartupTime;
        $this->videoTitle = $videoTitle;
        $this->videoVariantId = $videoVariantId;
        $this->videoVariantName = $videoVariantName;
        $this->viewEnd = $viewEnd;
        $this->viewMaxPlayheadPosition = $viewMaxPlayheadPosition;
        $this->viewPageUrl = $viewPageUrl;
        $this->viewPlayingTime = $viewPlayingTime;
        $this->viewSeekedCount = $viewSeekedCount;
        $this->viewSeekedDuration = $viewSeekedDuration;
        $this->viewSessionId = $viewSessionId;
        $this->viewStart = $viewStart;
        $this->viewTotalContentPlaybackTime = $viewTotalContentPlaybackTime;
        $this->viewerId = $viewerId;
        $this->watchTime = $watchTime;
    }
}