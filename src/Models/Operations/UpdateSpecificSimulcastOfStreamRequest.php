<?php



declare(strict_types=1);

namespace FastPix\Sdk\Models\Operations;

use FastPix\Sdk\Models\Components;
use FastPix\Sdk\Utils\FastPixMetadata;
class UpdateSpecificSimulcastOfStreamRequest
{
    /**
     * Upon creating a new live stream, FastPix assigns a unique identifier to the stream.
     *
     * @var string $streamId
     */
    #[FastPixMetadata('pathParam:style=simple,explode=false,name=streamId')]
    public string $streamId;

    /**
     * When you create the new simulcast, FastPix assign a universal unique identifier which can contain a maximum of 255 characters.
     *
     * @var string $simulcastId
     */
    #[FastPixMetadata('pathParam:style=simple,explode=false,name=simulcastId')]
    public string $simulcastId;

    /**
     *
     * @var Components\SimulcastUpdateRequest $simulcastUpdateRequest
     */
    #[FastPixMetadata('request:mediaType=application/json')]
    public Components\SimulcastUpdateRequest $simulcastUpdateRequest;

    /**
     * @param  string  $streamId
     * @param  string  $simulcastId
     * @param  Components\SimulcastUpdateRequest  $simulcastUpdateRequest
     * @phpstan-pure
     */
    public function __construct(string $streamId, string $simulcastId, Components\SimulcastUpdateRequest $simulcastUpdateRequest)
    {
        $this->streamId = $streamId;
        $this->simulcastId = $simulcastId;
        $this->simulcastUpdateRequest = $simulcastUpdateRequest;
    }
}