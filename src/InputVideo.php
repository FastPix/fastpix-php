...
class InputVideo
{
     ...
     */
    public function createMedia(Components\CreateMediaRequest $request, ?Options $options = null): Operations\CreateMediaResponse
    {
        $retryConfig = null;
        if ($options) {
        ...
        $hookContext = new HookContext($this->sdkConfiguration, $baseUrl, 'create-media', [], $this->sdkConfiguration->securitySource);
        ...
        if (Utils\Utils::matchStatusCodes($statusCode, ['201'])) {
            if (Utils\Utils::matchContentType($contentType, 'application/json')) {
                ...
                $obj = $serializer->deserialize($responseData, '\FastPix\Sdk\Models\Components\CreateMediaSuccessResponse', 'json', DeserializationContext::create()->setRequireAllRequiredProperties(true));
    ...
    public function directUploadVideoMedia(?Operations\DirectUploadVideoMediaRequest $request = null, ?Options $options = null): Operations\DirectUploadVideoMediaResponse
    {
        ...
        $httpOptions['headers']['user-agent'] = $this->sdkConfiguration->userAgent;
        $httpRequest = new \GuzzleHttp\Psr7\Request('POST', $url);
        $hookContext = new HookContext($this->sdkConfiguration, $baseUrl, 'direct-upload-video-media', [], $this->sdkConfiguration->securitySource);
        $httpRequest = $this->sdkConfiguration->hooks->beforeRequest(new Hooks\BeforeRequestContext($hookContext), $httpRequest);
        $httpOptions = Utils\Utils::convertHeadersToOptions($httpRequest, $httpOptions);
        $httpRequest = Utils\Utils::removeHeaders($httpRequest);
        try {
     ...
     * Direct upload
     *
     * Uploads media files directly to FastPix without using URLs. This endpoint allows you to upload video, audio, or other media files directly from your local system or application.
     *
     * @param  Components\DirectUploadRequest  $requestBody
     * @param  ?Options  $options
     * @return Operations\DirectUploadResponse
     * @throws \FastPix\Sdk\Models\Errors\APIException
     */
    public function directUpload(Components\DirectUploadRequest $requestBody, ?Options $options = null): Operations\DirectUploadResponse
    {
        $retryConfig = null;
        if ($options) {
            $retryConfig = $options->retryConfig;
        ...
        $multipartData = Utils\Utils::serializeRequestBody($requestBody, 'request', 'multipart');
        $request = new \GuzzleHttp\Psr7\Request('POST', Utils\Utils::generateUrl($this->sdkConfiguration->getTemplatedServerUrl(), '/on-demand/upload'), [
            'user-agent' => $this->sdkConfiguration->userAgent,
            'Accept' => 'application/json',
        ]);

        $hookCtx = new Hooks\HookContext($this->sdkConfiguration, $this->sdkConfiguration->getTemplatedServerUrl(), 'directUpload', [], $this->sdkConfiguration->securitySource);

        $request = $this->sdkConfiguration->hooks->beforeRequest(new Hooks\BeforeRequestContext($hookCtx), $request);

        // For multipart requests, we need to use a different approach
        if ($multipartData && isset($multipartData['multipart'])) {
            $httpResponse = $this->sdkConfiguration->client->request('POST', Utils\Utils::generateUrl($this->sdkConfiguration->getTemplatedServerUrl(), '/on-demand/upload'), [
                'headers' => [
                    'user-agent' => $this->sdkConfiguration->userAgent,
                    'Accept' => 'application/json',
                ],
                'multipart' => $multipartData['multipart'],
            ]);
        } else {
            $httpResponse = $this->sdkConfiguration->client->send($request);
        }

        $httpResponse = $this->sdkConfiguration->hooks->afterSuccess(new Hooks\AfterSuccessContext($hookCtx), $httpResponse);
        ...
        if (Utils\Utils::matchStatusCodes($statusCode, ['201'])) {
            if (Utils\Utils::matchContentType($contentType, 'application/json')) {
                ...
                $serializer = Utils\JSON::createSerializer();
                $responseData = (string) $httpResponse->getBody();
                $obj = $serializer->deserialize($responseData, '\FastPix\Sdk\Models\Operations\DirectUploadResponse', 'json', DeserializationContext::create()->setRequireAllRequiredProperties(true));
                $obj->rawResponse = $httpResponse;
